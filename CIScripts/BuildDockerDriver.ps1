...
& "$Env:GOPATH/bin/go-msi" make --msi DockerDriver.msi --arch x64 --version 0.1 --src template --out $pwd/gomsi
Pop-Location

Move-Item $srcPath/DockerDriver.msi ./

$cerp = Get-Content $Env:CERT_PASSWORD_FILE_PATH
& $Env:SIGNTOOL_PATH sign /f $Env:CERT_PATH /p $cerp DockerDriver.msi
